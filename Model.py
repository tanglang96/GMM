                ...
                for j in range(img.shape[1]):
                    ...
                    for k in range(K):
                        if self.check(img[i][j],self.g_mat.mat[i][j][k]):
                            ...
                            u,sigma,x=self.g_mat.mat[i][j][k].u,self.g_mat.mat[i][j][k].sigma,img[i][j].astype(np.float)
                            delta=x-u
                            self.g_mat.mat[i][j][k].u=u+m*(self.alpha/(self.g_mat.weight[i][j][k]+epsilon))*delta
                            self.g_mat.mat[i][j][k].sigma=sigma+m*(self.alpha/(self.g_mat.weight[i][j][k]+epsilon))*(np.matmul(delta,delta.T)-sigma)
                        else:
                            m=0
                            self.g_mat.weight[i][j][k]=self.g_mat.weight[i][j][k]+self.alpha*(m-self.g_mat.weight[i][j][k])
                    if flag==0:
